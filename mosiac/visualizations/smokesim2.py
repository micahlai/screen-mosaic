"""Smoke 2 — curl-noise ambient flow + explicit cursor push/swirl.

(The original "Smoke" is a true stable-fluids solver with emergent eddies; this
variant fakes the swirl with a tangential vortex term around the cursor.)
...
@register("smoke2", "Smoke 2 (curl)")
class SmokeSim2(Visualization):
    ...
    # cursor interaction (the preview window drives this)
    POINTER_RADIUS = 0.10           # influence radius, fraction of the grid long side
    POINTER_PUSH = 1.5              # how hard the cursor pushes smoke along its motion
    POINTER_SWIRL = 2.0             # tangential swirl around the cursor

    def __init__(self, width, height):
        super().__init__(width, height)
        self._ptr = None            # (nx, ny, nvx, nvy) cursor force, or None
        if self.w >= self.h:
    ...
    def set_pointer(self, ptr):
        """Set the cursor force (nx, ny, nvx, nvy) in [0,1] field coords, or None."""
        self._ptr = ptr
    ...
    def _apply_pointer(self):
        ...
        # push the smoke along the cursor's motion
        self.vx = self.vx + (nvx * self.gw) * self.POINTER_PUSH * fall
        self.vy = self.vy + (nvy * self.gh) * self.POINTER_PUSH * fall
        # swirl: tangential rotation around the cursor, scaled by cursor speed
        speed = float((nvx * nvx + nvy * nvy) ** 0.5)
        r = torch.sqrt(dx * dx + dy * dy) + 1.0
        s = speed * max(self.gw, self.gh) * self.POINTER_SWIRL
        self.vx = self.vx + (-dy / r)[None, None] * s * fall
        self.vy = self.vy + (dx / r)[None, None] * s * fall
    ...
    def step(self):
        ...
        if self._ptr is not None:           # cursor pushes/swirls the smoke
            self._apply_pointer()
